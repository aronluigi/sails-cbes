    ...
    var adapter = {
        ...
        create: function (connection, collection, values, callback) {
            ...
            for (var i in values) {
                ...
                if (schema.hasOwnProperty('ttl') &&
                    schema.ttl.hasOwnProperty('defaultsTo') &&
                    typeof schema.ttl.defaultsTo == 'number')
                {
                    if (val.hasOwnProperty('ttl') && typeof val.ttl == 'number') {
                        ttl = val.ttl;
                    } else {
                        ttl = schema.ttl.defaultsTo;
                    }
                }

                db.create(collection, val, ttl, function(err, _vals){
                    if (err){
                        return callback(err, false);
                    }
        ...
        reindex : function(connection, collection, callback) {
            var connectionObject = connections[connection],
                db = require('./db')(connectionObject.connection);

            db.reindex(collection, this.mapping, callback);
        }
    };
