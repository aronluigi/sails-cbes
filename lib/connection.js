...
Connection.prototype._initElasticSearch = function(allMappings, callback){
    var self = this,
        ...
        es = new elasticSearch.Client({host: config.host, log: config.log, requestTimeout: config.requestTimeout});
    ...
    es.indices.exists({index: config.index}, function(err, res, stat){
        ...
        if (res === false) {
            ...
            if (config.version === '2.1') {
                params.body.settings.max_result_window = config.maxResultWindow;
            }
