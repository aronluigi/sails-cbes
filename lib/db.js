        ...
        aggregate: function(collection, filterQuery, callback){
            ...
            es.search(query, function(err, res){
                ...
                if (res.hasOwnProperty('aggregations')) {
                    ...
                    console.log('---------------')
                    console.log(JSON.stringify(res.aggregations, null, 4));*/
                } else {
                    callback(null, null);
                }
            });
                    ...
                    self.getCollection(collection, function(err, res){
                        ...
                        try {
                            Object.keys(res).forEach(function (key) {
                                var data = res[key].doc;

                                if (_ttl) {
                                    data._ttl = Math.round((res[key].meta.expiration * 1000) - (new Date().getTime()));
                                ...
                                es.create(esEntry(data), function (err, res) {
                                    if (err) {
                                        throw new Error(err);
                                    }
                                });
