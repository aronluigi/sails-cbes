...
module.exports = function(connectionObject){
    ...
    return {
         ...
         * @param callback
         */
        create: function(collection, data, ttl, callback){
            cb.counter(collection + ":count" , 1, {initial: 0, offset: 1}, function(err, res){
                ...
                if (ttl) {
                    meta.expiry = Math.floor((new Date().getTime() + ttl + ttlInterval) / 1000);
                }
        ...
        reindex: function(collection, mapping, callback){
            var self = this,
                index = connectionObject.config.es.index,
                deleteParam = {
                    index: index,
                    ...
                    self.getCollection(collection, function(err, res){
                        ...
                        try {
                            Object.keys(res).forEach(function (key) {
                                es.create(esEntry(key, res[key]), function (err, res) {
                                    if (err) {
                                        throw err;
                                    }
                                });
