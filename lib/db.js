        ...
        create: function(collection, data, callback){
            cb.counter(collection + ":count" , 1, {initial: 0, offset: 1}, function(err, res){
                ...
                if (data.hasOwnProperty("ttl")) {
                    ttl = _.clone(data["ttl"]);
                    delete data["ttl"];
                }

                //ttl should be a number representing milliseconds
                if(ttl && typeof ttl !== "number") return callback(new Error("Type of attribute 'ttl' is not 'number'."), false);

                //added the ttlInterval to make sure the record is deleted from elasticsearch first
                if (ttl) meta.expiry = Math.floor((new Date().getTime() + ttl + ttlInterval)/ 1000);
